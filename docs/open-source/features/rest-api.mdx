...
- **Target-scoped reads:** Use `/target` to retrieve user-level, agent-level, or run-level memories.
...
| `GET` | `/memories` | Get memories by `user_id`, `agent_id`, or `run_id` |
| `GET` | `/target` | Get user-level memories by `user_id`, agent-level memories by `user_id` and `agent_id`, or run-level memories by `run_id` |
