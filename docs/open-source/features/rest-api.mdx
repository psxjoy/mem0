...
- **Target-scoped reads:** Use `/target` to retrieve only user-level memories for a user, or only agent-level memories for a user-agent pair.
...
| `GET` | `/target` | Get only user-level memories by `user_id`, or agent-level memories by `user_id` and `agent_id` |
